...
async function fetchActivity(vault: Contract): Promise<ActivityEntry[]> {
  ...
  try {
    ...
    for (const e of dep) {
      ...
      events.push({ type: 'Deposited', label: 'LP Deposit', amount: f18(args.usdcAmount) + ' mUSDC', from: args.user, block: e.blockNumber ?? 0 })
    }
    for (const e of wit) {
      const args = (e as any).args
      events.push({ type: 'Withdrawn', label: 'LP Withdraw', amount: f18(args.usdcAmount) + ' mUSDC', from: args.user, block: e.blockNumber ?? 0 })
    }
    for (const e of pro) {
      const args = (e as any).args
      events.push({ type: 'ProtocolDeposit', label: 'Protocol Fee', amount: f18(args.amount) + ' mUSDC', from: args.from, block: e.blockNumber ?? 0 })
    }
    for (const e of bai) {
      const args = (e as any).args
      events.push({ type: 'Bailout', label: 'Bailout Paid', amount: f18(args.amount) + ' mUSDC', from: args.trader, block: e.blockNumber ?? 0 })
    }
