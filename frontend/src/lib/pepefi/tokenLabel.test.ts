...
describe('token labels', () => {
  it('平台保證金一律是 mUSDC —— 和鏈上 MockUSDC 的 symbol 一致', () => {
    expect(STABLE_LABEL).toBe('mUSDC')
  })

  it('x402 結算幣永遠是「官方 USDC」,不能被 STABLE_LABEL 取代', () => {
    // x402 走 Circle 官方 USDC（EIP-3009），那是真錢。把它和測試用的 mUSDC
  ...
  it('withStable 產生「數字 + 空白 + 標籤」', () => {
    expect(withStable('123.45')).toBe('123.45 mUSDC')
    expect(withStable(0)).toBe('0 mUSDC')
  })
})
