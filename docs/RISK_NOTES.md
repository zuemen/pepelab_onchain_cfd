...
- **遷移路徑（已備妥、dry-run 通過，等手動 broadcast）**：因 oracle 位址在
  `PerpetualExchange` constructor 為 immutable，無法就地切換 → 用 `DeployWithPyth`
  **重部署**一條指向 `AggregatorOracle`(Pyth+Chainlink、median/degrade) 的新
  `PerpetualExchange`，並**遷移既有保證金／session／倉位**到新合約。`DeployWithPyth`
  已 dry-run 模擬成功（全棧部署+wiring 無 revert）；Pyth BTC/ETH feed id 為通用值、
  Base Sepolia Pyth 合約 `0xA2aa…5729`，RWA(AAPL/TSLA/GOLD) feed id 由 `PYTH_ID_*` env 提供。
  **刻意尚未 broadcast**（重部署會清掉現有 demo 的倉位／session／保證金）。
- 雙軌定位：**合成資產 demo 用 MockOracle（現 live）、RWA 正式定價用 Pyth/Chainlink（新部署）**。
  **現 live 仍測試網 owner-oracle；正式版切 Aggregator(Pyth+Chainlink)。**

### ✅ 資金費守恆轉移（正式版已修；live demo 仍舊版）
- **正式版（新部署）已改為多空之間守恆轉移**：`PerpetualExchange` 改用 per-side 雙
  funding 索引（long/short），每結算週期擁擠側付、對側收**等額**——
  `Σ 多方付 == Σ 空方收`（取整餘塵歸池、利償付），單邊市場無對手則不計 funding。
  見 `test/Funding.t.sol` 守恆／單邊測試與 `SecurityFixes.t.sol`（forge test 全綠）。
- ⚠ **目前 live demo 交易所（`0xEf75…`）仍是舊版單索引、非完全守恆**（多空 notional
  不等時差額由池吸收）。守恆版要生效需如下節 oracle 一併**重部署**（未執行）。
- 舊版行為（簡化）：`funding = notional × indexDiff`，單一索引同時套用多空兩側。
...
### 🟡 mark vs index
- 設計上 PnL／清算可用 **mark 價**估值：`mark = index × (1 + OI失衡 × cap)`。
- **現況 `markPremiumCapBps = 0` → mark == index**（PnL 用純指數價，安全；薄 OI 不會被
  少量倉位操縱 mark）。要讓 mark 反映 OI 需先有足夠真實 OI 才開 cap，屬 roadmap。
...
_最後更新：2026-06-27。funding 守恆轉移（正式版）、ADL 已於 live 開啟、oracle Aggregator
遷移已 dry-run；對應 commit 見 git log / PR（feat/funding-conservation）。_
