...
# script now writes three files: one table of data keyed by id, and one module
# of text per locale keyed by the same id. During the extraction phase both
# locales carry the same original Chinese; translating `en` is a separate step.
...
CATALOG_HEADER = {
    ...
    'en': [
        ...
        '',
        '/**',
        ' * 見 `../zh-TW/pepeStageSkins.ts`。搬移階段逐字複製原文。',
        ' *',
        ' * GENERATED by scripts/import-skins.py — edit that, not this.',
        ' */',
...
def main() -> None:
    ...
    for stage in sorted(pools):
        ...
        for i, (slug, name) in enumerate(entries):
            ...
            lines.append(
                "    { id: 's%d_%s', stage: %d, rarity: '%s', "
                "price: %d, emoji: '%s', image: '/pepe-skins/stage%d/%s.webp', "
                "groundY: %s },"
                % (stage, slug, stage, rarity, PRICE[rarity] + stage * 40,
                   EMOJI.get(slug, '🐸'), stage, slug, anchors[slug]['groundY']))
            names.append("  s%d_%s: '%s'," % (stage, slug, name))
        lines.append('  ],')
    lines.append(FOOTER)
