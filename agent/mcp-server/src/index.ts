...
server.tool(
  "open_position",
  "【寫】在指定 session 限額內為 session 使用者開一筆受限部位（受 per-trade cap / budget / leverage cap / expiry 約束）。需 AGENT_PRIVATE_KEY + SESSION_MANAGER_ADDRESS；缺則回明確錯誤。**必須**帶 authVcJson（使用者簽發的授權 VC）→ 下單前驗簽+鏈上交叉比對，不符即拒絕。回傳 tx hash 與 positionId。",
  {
    sessionId: z.number().int().nonnegative().describe("鏈上 session id"),
    asset: z.string().describe("資產代號，如 sBTC / sETH / sAAPL"),
...
server.tool(
  "close_position",
  "【寫】平掉指定 session 使用者的一筆部位（會實現損益，授權要求與開倉對稱）。需 AGENT_PRIVATE_KEY + SESSION_MANAGER_ADDRESS 與 authVcJson；缺則回明確錯誤。回傳 tx hash。",
  {
    sessionId: z.number().int().nonnegative().describe("鏈上 session id"),
